...
bool position(Board *b, char *text, Position *p) {
  ...
  if( text[2] != '\0') return false;
    ...
    if ( b->cells[num1][num2] != '.') return false;
...
bool won(Board *b) {
  ...
  if( line(b->cells[0][2],b->cells[1][1],b->cells[2][0]) == true){
  return true;
  }
...
void display(Board b) {
   for (int i= 0 ; i<=2 ; i ++){
       printf(" %c| %c| %c\n",b.cells[0][i],b.cells[1][i],b.cells[2][i]);
       printf("---+---+---\n");
